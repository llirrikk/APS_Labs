...
4. В уже созданном проекте создать новый модуль для АЛУ (пример создания проекта и модулей в [`Создание базового проекта с прошивкой ПЛИС в Vivado`](https://github.com/MPSU/APS/blob/technical/Other/Vivado-trainer.md))
5. Описать АЛУ на языке verilog (практическая работа)
6. Реализовать тестовое окружение для АЛУ (практическая работа, пример в [`Тестовые окружения`](https://github.com/MPSU/APS/blob/technical/Other/Testbench.md))
...
Существует несколько подходов к реализации АЛУ, отличающиеся внутренней организацией. Некоторые из подходов освещаются на [`лекции 3`](https://github.com/MPSU/APS/blob/technical/Lectures1/03.%20Digital%20arithmetics.md). В лабораторных работах применяется повсеместно используемый подход мультиплексирования операций, то есть, подключение нескольких операционных устройств (которые выполняют какие-то операции, например сложения, логическое И и т.п.) к мультиплексору, который будет передавать результат нужного операционного устройства на выходы АЛУ.
...
![АЛУ MIPS пример](/media/labs/alu_3.png)
...
![Мультиплексор2в1](/media/labs/alu_8.png)
...
![Мультиплексор](/media/labs/alu_6.png)
...
После реализации АЛУ также потребуется удостовериться в правильности его работы, для этого будет нужно создать тестовое окружение, пример которого описан в отдельной статье [`Тестовые окружения`](https://github.com/MPSU/APS/blob/%D0%98%D0%9A%D0%A2/Other/Testbench.md).
...
![АЛУ RISC-V](/media/labs/alu_4.png)
...
![Управляющие коды АЛУ RISC-V](/media/labs/alu_5.png)
...
![Готовое АЛУ](/media/labs/alu_7.png)
...
После реализации модуля АЛУ его нужно будет проверить с помощью [`тестового окружения`](https://github.com/MPSU/APS/blob/%D0%98%D0%9A%D0%A2/Other/Testbench.md).
